...
<section class="reservation">
      ...
      <h3 class="custom-heading"><span>Reservation Details</span></h3>
          ...
          <?php echo form_open('', $attr);?>
       		<div class="col-xs-12 reservation-form">
				  ...
				  <div class="form-group">
				    ...
				    <div class="col-sm-10">
				      <input type="text" class="form-control" id="" placeholder="House no, Street, Brgy, City" name="venue" required="" value="<?php echo $schedule->venue;?>">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <select class="form-control" name="event_type" required="">
                ...
                <option value="<?php echo $service->id;?>" <?php if($schedule->event_type == $service->id){ echo "selected"; }?>><?php echo $service->name;?></option>
                <?php endforeach;?>
                <?php endif;?>
              </select>
            </div>
              ...
              <?php $min_date =  date('Y-m-d', strtotime('+2 months'));?>
				      <input type="date" class="form-control" value="<?php echo date('Y-m-d', strtotime($schedule->event_date));?>" name="event_date" min="<?php echo $min_date;?>" required="">
              ...
              <select class="form-control" name="motif" id="motifSelect">
                ...
                <!-- <optgroup label="Motif for debut"> -->
                  <?php foreach($motif_debut as $debut):?>
                    <option value="<?php echo $debut;?>" <?php if($schedule->motif == $debut){ echo 'selected'; }?>><?php echo $debut;?></option>
                  <?php endforeach;?>
                <!-- </optgroup> -->
                <!-- <optgroup label="Motif for  kiddie /christening party"> -->
                  <?php foreach($motif_kiddie as $kiddie):?>
                    <option value="<?php echo $kiddie;?>" <?php if($schedule->motif == $kiddie){ echo 'selected'; }?>><?php echo $kiddie;?></option>
                  <?php endforeach;?>
                <!-- </optgroup>                   -->
                <!-- <optgroup label="For all events motif"> -->
                  <?php foreach($motif_all as $all):?>
                    <option value="<?php echo $all;?>" <?php if($schedule->motif == $all){ echo 'selected'; }?>><?php echo $all;?></option>
                  <?php endforeach;?>
                <!-- </optgroup>  -->
                </option>
              </select>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <input type="number" class="form-control" placeholder="No. of Guest" name="no_guests" required="" min="100" id="noGuests" value="<?php echo $schedule->no_guests;?>">
        ...
        <div class="col-xs-12 reservation-form" style="margin-top: 20px;">
          <div class="form-group">
                    ...
                    <div class="col-xs-4" style="display:<?php if($package->name == 'AMENITIES'){ echo 'none'; } ?>">
                      <div class="card card-2">
                        ...
                        <div class="card-details">
                          <a href="<?php echo base_url();?>packages/view/<?php echo $package->id;?>" target="_blank"><p style="white-space: pre-line;">See details</p></a>
                          <input type="radio" name="package" value="<?php echo $package->id?>" required <?php if($schedule->package == $package->id){ echo 'checked'; } ?>> <small>Select this package</small>
                        </div>
                      </div>
                    </div>
                  <?php endforeach;?>
            ...
            <div class="col-sm-10">
              ...
              <?php 
                $service_style = explode(',' , $schedule->service_style);
              ?>
              <input type="checkbox" name="service_style[]" class="service_checkbox" style="margin-right: 10px;" value="Plated service" <?php if(in_array('Plated service', $service_style)){ echo 'checked'; }?>>Plated service<small style="margin-left: 25px;flex: 0 0 100%;display: inline-block;"><i>-Selection of pre-plated foods, such as entrees, sandwich plates and salad plates, set on a buffet table. They may also be placed on a roll-in (a i.e., rolling cart or table) and then moved into the function room at the designated time. Because of individual plates, trays are usually used</i></small>
              </div>
              <div style="display: flex; flex-wrap: wrap;">
              <input type="checkbox" name="service_style[]" class="service_checkbox" style="margin-right: 10px;" value="Family or lauriat" <?php if(in_array('Family or lauriat', $service_style)){ echo 'checked'; }?>>Family or lauriat <small style="margin-left: 25px;flex: 0 0 100%;display: inline-block;"><i>-Guests are seated. Large serving platters and bowls are filled with foods in the kitchen and set on the dining tables by servers. Guests help themselves from a lazy Susan or they pass the foods to each other.</i></small>
              </div>
              <div style="display: flex; flex-wrap: wrap;">
              <input type="checkbox" name="service_style[]" class="service_checkbox" style="margin-right: 10px;" value="Buffet" <?php if(in_array('Buffet', $service_style)){ echo 'checked'; }?>>Buffet<small style="margin-left: 25px;flex: 0 0 100%;display: inline-block;"><i>-is a system of serving meals in which food is placed in a public area where the diners serve themselves</i></small>
              </div>
            </div>
          </div>
          ...
          <div class="form-group flex align-center col-xs-12">
            ...
            <div class="col-sm-10">
              <?php 
                $additionals = explode(',' , $schedule->additionals);
              ?>
              <input type="checkbox" name="additionals[]" class="additionals_checkbox" style="margin-right: 10px;" value="Sounds and light with projector" <?php if(in_array('Sounds and light with projector', $additionals)){ echo 'checked'; }?>>Sounds and light with projector - 5,000
              <input type="checkbox" name="additionals[]" class="additionals_checkbox" style="margin-right: 10px;margin-left: 10px;" value="Photoboth for 2 hRs" <?php if(in_array('Photoboth for 2 hRs', $additionals)){ echo 'checked'; }?>>Photoboth for 2 hRs - 4,000
              <input type="checkbox" name="additionals[]" class="additionals_checkbox" style="margin-right: 10px;margin-left: 10px;" value="Photo and video" <?php if(in_array('Photo and video', $additionals)){ echo 'checked'; }?>>Photo and video - 3,500
              <input type="checkbox" name="additionals[]" class="additionals_checkbox" style="margin-right: 10px;margin-left: 10px;" value="Emcee" <?php if(in_array('Emcee', $additionals)){ echo 'checked'; }?>>Emcee - 2,000
              ...
              <input type="checkbox" name="additionals[]" class="additionals_checkbox" style="margin-right: 10px;margin-left: 10px;" value="Motif" <?php if(in_array('Motif', $additionals)){ echo 'checked'; }?>>Motif - 5,000
            </div>
          </div>
                  ...
                  <?php foreach($res as $r):?>
                    ...
                    <div class="col-xs-4">
                      <div class="card card-2">
                        ...
                        <div class="card-details">
                          <?php $foods = explode(',' , $schedule->additional_foods);?>
                          <input type="checkbox" name="foods[]" value="<?php echo $r->id?>" <?php if(in_array($r->id, $foods)){ echo 'checked'; } ?>> <small>Select food</small>
                        </div>
                      </div>
                    </div>
